...
import re
print(re.match('www', 'www.lsaiah.cn').span())  # 在起始位置匹配
print(re.match('com', 'www.lsaiah.cn'))         # 不在起始位置匹配
...
print(re.search('www', 'www.lsaiah.cn').span())  # 在起始位置匹配
print(re.search('com', 'www.lsaiah.cn').span())  # 不在起始位置匹配
# 输出结果
(0, 3)
(11, 14)
```
...
在字符串中找到正则表达式所匹配的所有子串，并返回一个列表，如果没有找到匹配的，则返回空列表。 match 和 search 是匹配一次 findall 匹配所有。语法格式为：
...
result1 = re.findall(r'\d+','lsaiah 123 kwaiyu 456')
 
pattern = re.compile(r'\d+')   # 查找数字
result2 = pattern.findall('lsaiah 123 kwaiyu 456')
result3 = pattern.findall('lsai88ah123kwaiyu456', 0, 10)
...
```python
>>>import re
>>> re.split('\W+', 'lsaiah, lsaiah, lsaiah.')
['lsaiah', 'lsaiah', 'lsaiah', '']
>>> re.split('(\W+)', ' lsaiah, lsaiah, lsaiah.') 
['', ' ', 'lsaiah', ', ', 'lsaiah', ', ', 'lsaiah', '.', '']
>>> re.split('\W+', ' lsaiah, lsaiah, lsaiah.', 1) 
['', 'lsaiah, lsaiah, lsaiah.']
...
def name_of_email(addr):
    re_mail = re.compile(r'^<?(\w+\s?\w+)?>?(\s?\w+)?@(\w+).(org)$')
